  ...
  try {
    ...
    if (process.env.SENDGRID_API_KEY) {
      try {
        ...
        await sgMail.send({ to: contactEmail, from: process.env.SENDGRID_FROM_EMAIL || 'hello@nexaweb.com', subject: 'Order Confirmed — NexaWeb', text: `Hi ${contactName}, thanks for your order! Track: ${clientUrl}/order/track/${trackingLink}` });
        await sgMail.send({ to: process.env.ADMIN_EMAIL || 'admin@nexaweb.com', from: process.env.SENDGRID_FROM_EMAIL || 'hello@nexaweb.com', subject: `New Order: ${packageName} — ${businessName}`, text: `Order #${trackingLink}: ${businessName} (${packageName}). Contact: ${contactName} <${contactEmail}>` });
